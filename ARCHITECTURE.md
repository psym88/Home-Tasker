...
Collapsible boxes remove redundant vertical padding and keep the clickable header aligned with its visible area. The optional start boundary is the final planning control before the generated text, uses the same chevron treatment, and opens automatically when populated. Its visible text triggers a visually hidden native date input and exposes a clear action only when populated. The editor initially requests four due dates and increments the backend sequence count by one for each "Show more" action placed directly below the list. Preview dates combine a localized weekday with the locale-specific numeric date. The backend uses the same scheduler for previews, task creation, schedule updates, and persisted completion dates. Icon actions use circular theme-aware hover and focus treatments.
