...
Typography follows Home Assistant font, size, weight, line-height, and color variables. The small font size is reserved for the due-date preview; other supporting text uses the normal body size. Main-list group names use the large heading size, while task names use the normal body size and weight. Action buttons share a 44-pixel minimum control height, while icon actions use a matching square footprint.
