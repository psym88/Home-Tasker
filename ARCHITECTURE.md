...
The task viewer is registered as a standalone Home Assistant dialog element and opened with a composed, bubbling `show-dialog` event. It uses Home Assistant's adaptive dialog and generic confirmation dialog but has no Browser Mod dependency. The task editor remains an integration-owned modal workflow.
