...
Collapsible boxes remove redundant vertical padding and keep the clickable header aligned with its visible area. The editor requests its second and third due-date previews through an admin-only WebSocket command that calls `scheduler.py` twice in sequence, so previews and persisted completion dates share the authoritative backend calculation. Icon actions use circular theme-aware hover and focus treatments.
