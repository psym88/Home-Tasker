...
export class HomeTaskerCard extends HomeTaskerBase {
  ...
  render(){
    ...
    if(!this.shadowRoot.querySelector(".card-root"))this.shadowRoot.innerHTML=`${this.styles()}<style>:host{display:block}ha-card{overflow:hidden}.card-content{display:grid;gap:8px;padding:12px}.task-row{display:grid;grid-template-columns:minmax(0,1fr);align-items:center;min-height:52px;padding:4px 12px;border:1px solid var(--ha-card-border-color,var(--divider-color));border-radius:var(--ha-card-border-radius,12px);background:var(--ha-card-background,var(--card-background-color));cursor:pointer;transition:background-color 180ms ease-in-out}.task-row:hover{background:var(--secondary-background-color)}.task-row.editable{grid-template-columns:minmax(0,1fr) 44px}.task-name{color:var(--primary-text-color);font-weight:var(--ha-font-weight-normal,400)}.pills{display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-top:4px}.due-date.today{color:var(--warning-color,#f57c00)}.due-date.overdue{color:var(--error-color,#d32f2f)}.due-date.future{color:var(--success-color,#43a047)}.empty{padding:20px;color:var(--secondary-text-color);text-align:center}.add-task{box-sizing:border-box;display:flex;align-items:center;justify-content:flex-start;gap:7px;width:100%;min-height:52px;padding:4px 12px;border:1px dashed var(--divider-color);border-radius:var(--ha-card-border-radius,12px);background:transparent;color:var(--secondary-text-color);font:inherit;text-align:left}.add-task:hover,.add-task:focus-visible{background:rgba(var(--rgb-primary-text-color),.04);border-color:var(--secondary-text-color);outline:0}</style><div class="card-root"></div>`;
    const root=this.shadowRoot.querySelector(".card-root"),tasks=this.visibleTasks(),editable=canEditCard(this.config);
    root.innerHTML=dashboardCardBodyHtml(tasks.map(task=>dashboardTaskRowHtml(task,editable,this.relativeDate(task.due_date),dueStatus(task.due_date,this.today),this.users.find(user=>user.id===task.assignee_user_id)?.name||"",this.tagName(task))).join(""),editable,this.config.hide_card_background);
    root.insertAdjacentHTML("beforeend",'<style>.task-row{border-color:var(--divider-color)}.add-task{justify-content:center;text-align:center}</style>'+ROW_ACTION_MENU_STYLES+this.themeStyles()+`<style>${this.pillIconCss()}</style>`+this.iconHoverStyles());
...
export class HomeTaskerCardEditor extends HTMLElement {
  ...
  render(){if(!this.shadowRoot)return;this.shadowRoot.innerHTML=`<style>:host{display:block}.editor{display:grid;gap:16px}.field{display:grid;gap:7px}.choices{display:grid;gap:6px;max-height:180px;overflow:auto;padding:8px;border:1px solid var(--divider-color);border-radius:8px}.choices label,.toggle{display:flex;gap:8px;align-items:center}select,input[type=number]{box-sizing:border-box;width:100%;padding:9px;border:1px solid var(--divider-color);border-radius:8px;background:var(--card-background-color);color:var(--primary-text-color)}</style><div class="editor"><label class="field"><span>${t("card.mode")}</span><select name="mode"><option value="view" ${this.config.mode==="view"?"selected":""}>${t("card.mode_view")}</option><option value="edit" ${this.config.mode==="edit"?"selected":""}>${t("card.mode_edit")}</option></select></label><label class="toggle"><input name="hide_card_background" type="checkbox" ${this.config.hide_card_background?"checked":""}><span>${t("card.hide_background")}</span></label><label class="field"><span>${t("card.due_days")}</span><input name="due_days" type="number" min="0" step="1" value="${this.config.due_days??""}"></label><div class="field"><span>${t("card.groups")}</span><div class="choices groups">${this.checks(this.groups,this.config.group_ids,"groups")||t("group.none")}</div></div><div class="field"><span>${t("card.users")}</span><div class="choices users">${this.checks([{id:UNASSIGNED,name:t("task.unassigned")},...this.users],this.config.assignee_user_ids,"users")}</div></div></div>`;
