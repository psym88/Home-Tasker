...
export class HomeTaskerCard extends HomeTaskerBase {
  ...
  render(){
    ...
    if(!this.shadowRoot.querySelector(".card-root"))this.shadowRoot.innerHTML=`${this.styles()}<style>:host{display:block}ha-card{overflow:hidden}.card-content{display:grid;gap:8px;padding:12px}.task-row{display:grid;grid-template-columns:minmax(0,1fr);align-items:center;min-height:52px;padding:4px 12px;border:1px solid var(--divider-color);border-radius:10px;background:var(--secondary-background-color);cursor:pointer}.task-row:hover{background:rgba(var(--rgb-primary-text-color),.04)}.task-row.editable{grid-template-columns:minmax(0,1fr) 44px}.task-name{color:var(--primary-text-color);font-weight:var(--ha-font-weight-normal,400)}.due-label{display:inline-flex;margin-top:3px;font-size:var(--ha-font-size-s,12px);font-weight:var(--ha-font-weight-medium,500)}.due-label.today{color:var(--warning-color,#f57c00)}.due-label.overdue{color:var(--error-color,#d32f2f)}.due-label.future{color:var(--success-color,#43a047)}.empty{padding:20px;color:var(--secondary-text-color);text-align:center}.add-task{box-sizing:border-box;display:flex;align-items:center;justify-content:flex-start;gap:7px;width:100%;min-height:52px;padding:4px 12px;border:1px dashed var(--divider-color);border-radius:10px;background:transparent;color:var(--secondary-text-color);font:inherit;text-align:left}.add-task:hover,.add-task:focus-visible{background:rgba(var(--rgb-primary-text-color),.04);border-color:var(--secondary-text-color);outline:0}</style><div class="card-root"></div>`;
