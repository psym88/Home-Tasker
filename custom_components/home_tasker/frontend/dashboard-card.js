...
export function dashboardTaskRowHtml(task,editable,relativeDate,status){return `<div class="task-row${editable?" editable":""}" data-task="${esc(task.id)}" tabindex="0"><div><div class="task-name">${esc(task.name)}</div><div class="due-label ${esc(status)}">${esc(relativeDate)}</div></div>${editable?'<button type="button" class="edit-task-row icon" aria-label="Task bearbeiten" title="Task bearbeiten"><ha-icon icon="mdi:pencil"></ha-icon></button>':""}</div>`;}
...
export class HomeTaskerCard extends HomeTaskerBase {
  ...
  render(){
    if(!this.shadowRoot.querySelector(".card-root"))this.shadowRoot.innerHTML=`${this.styles()}<style>:host{display:block}ha-card{overflow:hidden}.card-content{display:grid;gap:8px;padding:12px}.task-row{display:grid;grid-template-columns:minmax(0,1fr);align-items:center;min-height:52px;padding:4px 12px;border:1px solid var(--divider-color);border-radius:10px;background:var(--secondary-background-color);cursor:pointer}.task-row:hover{background:rgba(var(--rgb-primary-text-color),.04)}.task-row.editable{grid-template-columns:minmax(0,1fr) 44px}.task-name{color:var(--primary-text-color);font-weight:var(--ha-font-weight-normal,400)}.due-label{display:inline-flex;margin-top:3px;font-size:var(--ha-font-size-s,12px);font-weight:var(--ha-font-weight-medium,500)}.due-label.today{color:var(--warning-color,#f57c00)}.due-label.overdue{color:var(--error-color,#d32f2f)}.due-label.future{color:var(--success-color,#43a047)}.empty{padding:20px;color:var(--secondary-text-color);text-align:center}.card-actions{display:flex;justify-content:flex-end;padding:8px 12px 12px}.add-task{display:flex;align-items:center;gap:7px;background:var(--primary-color);color:var(--text-primary-color,#fff)}</style><div class="card-root"></div><div class="dialogs"></div>`;
    const root=this.shadowRoot.querySelector(".card-root"),tasks=this.visibleTasks(),editable=canEditCard(this.config,this._hass);
    root.innerHTML=`<ha-card><div class="card-content">${tasks.length?tasks.map(task=>dashboardTaskRowHtml(task,editable,this.relativeDate(task.due_date),dueStatus(task.due_date,this.today))).join(""):'<div class="empty">Keine passenden Tasks</div>'}</div>${editable?'<div class="card-actions"><button type="button" class="add-task"><ha-icon icon="mdi:plus"></ha-icon><span>Task hinzufügen</span></button></div>':""}</ha-card>`;
