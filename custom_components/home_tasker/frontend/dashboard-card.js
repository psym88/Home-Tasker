...
export function dashboardTaskRowHtml(task,editable,relativeDate,status,assigneeName="",tagName=""){return `<div class="task-row${editable?" editable":""}" data-task="${esc(task.id)}" tabindex="0"><div><div class="task-name">${esc(task.name)}</div><div class="pills"><span class="pill ht-content-small due-date ${esc(status)}">${esc(relativeDate)}</span>${assigneeName?`<span class="pill ht-content-small"><ha-icon icon="mdi:account"></ha-icon>${esc(assigneeName)}</span>`:""}${tagName?`<span class="pill ht-content-small"><ha-icon icon="mdi:nfc"></ha-icon>${esc(tagName)}</span>`:""}</div></div>${editable?'<span class="row-action-slot"></span>':""}</div>`;}
export function dashboardCardBodyHtml(rows,editable){return `<ha-card style="--ha-card-border-width:0px;--ha-card-border-color:transparent;border:none!important;background:transparent!important;box-shadow:none!important"><div class="card-content">${editable?`<button class="add-task" type="button"><ha-icon icon="mdi:plus"></ha-icon><span>${t("card.add")}</span></button>`:""}${rows||`<div class="empty">${t("card.empty")}</div>`}</div></ha-card>`;}
...
export class HomeTaskerCard extends HomeTaskerBase {
  ...
  render(){
    if(!this.shadowRoot.querySelector(".card-root"))this.shadowRoot.innerHTML=`${this.styles()}<style>:host{display:block}ha-card{overflow:hidden}.card-content{display:grid;gap:8px;padding:0}.task-row{display:grid;grid-template-columns:minmax(0,1fr);align-items:center;min-height:52px;padding:4px 12px;border:1px solid var(--ha-card-border-color,var(--divider-color));border-radius:var(--ha-card-border-radius,12px);background:var(--ha-card-background,var(--card-background-color));cursor:pointer;transition:background-color 180ms ease-in-out}.task-row:hover{background:var(--secondary-background-color)}.task-row.editable{grid-template-columns:minmax(0,1fr) 44px}.task-name{font-weight:var(--ha-font-weight-normal,400)}.pills{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-top:4px}.due-date.today{color:var(--warning-color,#f57c00)}.due-date.overdue{color:var(--error-color,#d32f2f)}.due-date.future{color:var(--success-color,#43a047)}.empty{padding:20px;color:var(--secondary-text-color);text-align:center}.add-task{display:flex;align-items:center;box-sizing:border-box;width:100%;min-height:52px;gap:8px;padding:4px 12px;border:1px dashed var(--divider-color);border-radius:var(--ha-card-border-radius,12px);background:transparent;color:var(--primary-color);font:inherit;text-align:left;cursor:pointer}.add-task ha-icon{--mdc-icon-size:20px;width:20px;height:20px}.add-task:focus:not(:focus-visible){outline:none}</style><div class="card-root"></div>`;
