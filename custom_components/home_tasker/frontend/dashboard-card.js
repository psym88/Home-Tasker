...
export class HomeTaskerCardEditor extends HTMLElement {
  ...
  async load(){try{const data=await this._hass.connection.sendMessagePromise({type:"home_tasker/list"});this.users=data.users||[];}catch(error){console.error("Home Tasker card editor load failed",error);}this.render();}
