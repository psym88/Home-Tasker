...
export class HomeTaskerCard extends HomeTaskerBase {
  ...
  render(){
    ...
    if(!this.shadowRoot.querySelector(".card-root"))this.shadowRoot.innerHTML=`${this.styles()}<style>:host{display:block}ha-card{overflow:hidden}.card-content{display:grid;gap:8px;padding:12px}.task-row{--task-color:var(--state-inactive-color,var(--secondary-text-color));display:grid;grid-template-columns:minmax(0,1fr);align-items:center;min-height:52px;padding:4px 12px;border:1px solid transparent;border-radius:var(--ha-card-border-radius,12px);background:color-mix(in srgb,var(--task-color) 12%,var(--ha-card-background,var(--card-background-color)));cursor:pointer;transition:background-color 180ms ease-in-out}.task-row:has(.due-label.today){--task-color:var(--state-climate-heat-color,var(--warning-color,#f57c00))}.task-row:has(.due-label.overdue){--task-color:var(--state-alert-color,var(--error-color,#d32f2f))}.task-row:has(.due-label.future){--task-color:var(--state-active-color,var(--success-color,#43a047))}.task-row:hover{background:color-mix(in srgb,var(--task-color) 18%,var(--ha-card-background,var(--card-background-color)))}.task-row.editable{grid-template-columns:minmax(0,1fr) 44px}.task-name{color:var(--primary-text-color);font-weight:var(--ha-font-weight-normal,400)}.due-label{display:inline-flex;margin-top:3px;color:var(--task-color);font-size:var(--ha-font-size-s,12px);font-weight:var(--ha-font-weight-medium,500)}.empty{padding:20px;color:var(--secondary-text-color);text-align:center}.add-task{box-sizing:border-box;display:flex;align-items:center;justify-content:flex-start;gap:7px;width:100%;min-height:52px;padding:4px 12px;border:1px dashed var(--divider-color);border-radius:var(--ha-card-border-radius,12px);background:transparent;color:var(--secondary-text-color);font:inherit;text-align:left}.add-task:hover,.add-task:focus-visible{background:rgba(var(--rgb-primary-text-color),.04);border-color:var(--secondary-text-color);outline:0}</style><div class="card-root"></div>`;
    ...
    root.insertAdjacentHTML("beforeend",'<style>.add-task{justify-content:center;text-align:center}</style>'+ROW_ACTION_MENU_STYLES+this.themeStyles()+this.iconHoverStyles());
