...
export class HomeTaskerSettingsDialog extends HTMLElement {
  ...
  render(){this.shadowRoot.innerHTML=`${this.controller.typographyStyles()}<style>:host{color:var(--primary-text-color)}ha-adaptive-dialog{--dialog-content-padding:0}.content{padding:16px 24px 24px;overflow:auto}.details-content{display:flex;flex-direction:column;gap:12px;padding:0 16px 16px}.actions{display:flex;gap:8px;flex-wrap:wrap}.hint{margin:0;color:var(--secondary-text-color)}.status{min-height:20px}.status.error{color:var(--error-color)}input{display:none}</style><ha-adaptive-dialog width="medium" flexcontent><ha-icon-button slot="headerNavigationIcon" class="close" label="${esc(t("common.close"))}"><ha-icon icon="mdi:close"></ha-icon></ha-icon-button><span slot="headerTitle">${esc(t("settings.title"))}</span><div class="content"><ha-expansion-panel><span slot="header" class="ht-label-medium">${esc(t("settings.import_export"))}</span><div class="details-content"><p class="hint ht-content">${esc(t("settings.archive_hint"))}</p><div class="actions"><ha-button class="export" variant="brand"><ha-icon icon="mdi:archive-arrow-down-outline"></ha-icon> ${esc(t("settings.export"))}</ha-button><ha-button class="import" variant="danger"><ha-icon icon="mdi:archive-arrow-up-outline"></ha-icon> ${esc(t("settings.import"))}</ha-button></div><input type="file" accept=".zip,application/zip"><div class="status ht-content" role="status"></div></div></ha-expansion-panel></div></ha-adaptive-dialog>`;const dialog=this.shadowRoot.querySelector("ha-adaptive-dialog"),input=this.shadowRoot.querySelector("input");dialog.open=true;dialog.addEventListener("closed",()=>this.dialogClosed(),{once:true});this.shadowRoot.querySelector(".close").onclick=()=>this.closeDialog();this.shadowRoot.querySelector(".export").onclick=()=>this.exportArchive();this.shadowRoot.querySelector(".import").onclick=()=>this.chooseImport();input.onchange=()=>{const file=input.files?.[0];input.value="";this.importArchive(file);};}
}
