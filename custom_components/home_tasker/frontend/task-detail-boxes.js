...
export function taskFilesBoxHtml(controller,task,{editable=false,pending=new Set()}={}){
  const files=controller.attachments.filter(file=>file.task_id===task.id),rows=files.length?files.map(file=>`<div data-file="${esc(file.id)}" class="file-row ${pending.has(file.id)?"pending":""}" style="${editable?EDITOR_FILE_GRID:VIEWER_FILE_GRID}">${controller.fileLink(file)}<span class="file-size" style="justify-self:end;white-space:nowrap">${controller.size(file.size)}</span>${editable?`<ha-icon-button class="editor-action remove ${pending.has(file.id)?"":"danger"}" label="${pending.has(file.id)?t("file.undo_remove"):t("file.remove")}" title="${pending.has(file.id)?t("common.undo"):t("common.remove")}" style="justify-self:end;--ha-icon-button-size:32px"><ha-icon icon="mdi:${pending.has(file.id)?"undo":"delete"}"></ha-icon></ha-icon-button>`:""}</div>`).join(""):(editable?"":`<small class="ht-content">${L.noFiles}</small>`);
