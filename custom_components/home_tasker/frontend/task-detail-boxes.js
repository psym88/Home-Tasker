...
export function taskFilesBoxHtml(controller,task,{editable=false,pending=new Set()}={}){
  ...
  const content=editable?`<div class="file-list">${rows}</div><ha-selector class="file-upload-loader" hidden></ha-selector><ha-file-upload class="attachment-upload"></ha-file-upload>`:`<div class="file-list">${rows}</div>`;
