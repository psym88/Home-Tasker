...
export const withTaskList = Base => class extends Base {
  ...
  dueState(t){return !this.today||t.due_date>this.today?"ok":t.due_date===this.today?"today":"overdue";}
  ...
  render(){
    ...
    if(!this.shadowRoot.querySelector(".app")) this.shadowRoot.innerHTML=`${this.styles()}${DUE_DATE_STYLES}<style>.task:hover{background:${TASK_ROW_HOVER_BACKGROUND}}.task{background:${TASK_ROW_BACKGROUND};cursor:grab;user-select:none}.task.dragging{opacity:.45}.group-head.drop-target{outline:2px solid var(--primary-color);outline-offset:-2px;background:var(--secondary-background-color)}</style><div class="app"></div>`;
    ...
    app.insertAdjacentHTML("beforeend",ROW_ACTION_MENU_STYLES+this.themeStyles()+this.typographyStyles()+this.buttonThemeStyles()+this.iconHoverStyles()+this.groupListStyles()+`<style>.placeholder-add{justify-content:center;text-align:center}${this.pillIconCss()}</style>`);
    ...
    app.querySelector(".list-add").onclick=()=>this.taskEditor(null);
  ...
  filePill(file){const name=String(file.filename||""),parts=this.fileNameParts(name),url=this.signedFiles.get(file.id),content=`<ha-icon icon="mdi:paperclip"></ha-icon><span style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(parts.base)}</span><span style="flex:none;white-space:nowrap">${esc(parts.extension)}</span>`,style="display:inline-flex;align-items:center;gap:5px;max-width:160px;min-width:0;color:var(--primary-color);text-decoration:none";return url?`<button type="button" class="pill file-pill file-open" data-file-open="${esc(file.id)}" title="${esc(name)}" style="${style};min-height:0;padding:3px 8px;border:0">${content}</button>`:`<span class="pill file-pill" title="${esc(name)}" style="${style}">${content}</span>`;}
  taskRow(t){ const files=this.attachments.filter(a=>a.task_id===t.id),assignee=this.users.find(user=>user.id===t.assignee_user_id),tagName=this.tagName(t),filePills=files.map(file=>this.filePill(file)).join("");return `<div class="task" data-task="${t.id}" tabindex="0" style="display:grid;grid-template-columns:minmax(0,1fr) 44px;align-items:center;gap:4px;padding:5px 6px 7px 10px"><div class="task-body" style="display:block;min-width:0"><strong class="ht-content">${esc(t.name)}</strong><div class="pills" style="display:flex;gap:6px;flex-wrap:wrap;align-items:center"><span class="pill due-date ${this.dueState(t)}" title="${esc(this.date(t.due_date))}">${esc(this.relativeDate(t.due_date))}</span>${assignee?`<span class="pill"><ha-icon icon="mdi:account"></ha-icon>${esc(assignee.name)}</span>`:""}${tagName?`<span class="pill ht-content-small"><ha-icon icon="mdi:nfc"></ha-icon>${esc(tagName)}</span>`:""}</div>${filePills?`<div class="pills" style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-top:6px">${filePills}</div>`:""}</div>${rowActionButtonHtml("task",t.id)}</div>`; }
  wireGroup(el){ const g=this.groups.find(x=>x.id===el.dataset.group),head=el.querySelector(".group-head"),toggle=()=>{this.expanded.has(g.id)?this.expanded.delete(g.id):this.expanded.add(g.id);this.render();};head.onclick=toggle;head.onkeydown=e=>{if(e.key==="Enter"||e.key===" ")toggle();};const groupActions=el.querySelector('.row-action-toggle[data-action-kind="group"]');groupActions.onclick=e=>{e.stopPropagation();this.actionMenu(groupActions,()=>this.groupEditor(g),()=>this.deleteGroup(g));};const groupAdd=el.querySelector(".group-add");if(groupAdd)groupAdd.onclick=()=>this.taskEditor(g.id);el.querySelectorAll("[data-task]").forEach(row=>{const t=this.tasks.find(x=>x.id===row.dataset.task),taskActions=row.querySelector('.row-action-toggle[data-action-kind="task"]');row.onclick=()=>this.taskViewer(t);row.onkeydown=e=>{if(e.target===row&&(e.key==="Enter"||e.key===" "))this.taskViewer(t);};taskActions.onclick=e=>{e.stopPropagation();this.actionMenu(taskActions,()=>this.taskEditor(g.id,t),()=>this.deleteTask(t));};});this.wireFileOpeners(el); }
};
