...
export const withTaskList = Base => class extends Base {
  ...
  render(){
    ...
    app.innerHTML=`<style>main{padding-bottom:36px}.task{cursor:pointer;user-select:auto}.placeholder-add{display:flex;align-items:center;justify-content:flex-start;gap:7px;width:100%;min-height:52px;padding:4px 12px;border:1px dashed var(--divider-color);border-radius:10px;background:transparent;color:var(--secondary-text-color);font:inherit;text-align:left}.placeholder-add:hover,.placeholder-add:focus-visible{background:${TASK_ROW_HOVER_BACKGROUND};border-color:var(--secondary-text-color);outline:0}.list-add{margin-bottom:8px}.group-add{width:calc(100% - 20px);margin:8px 10px}.version{right:10px;left:auto;top:8px;bottom:auto}</style><main><header><div><h1>Home Tasker</h1><p>Wiederkehrende Aufgaben im Blick behalten</p></div></header>
      <nav><i></i><button class="link expand" style="color:${LIST_SECONDARY_ACTION_COLOR}">${this.groups.every(g=>this.expanded.has(g.id))?"Alle zuklappen":"Alle aufklappen"}</button><ha-select class="sort" label="Sortierung"><ha-list-item value="name">Name</ha-list-item><ha-list-item value="due">Fälligkeit</ha-list-item></ha-select></nav>
    ...
    app.insertAdjacentHTML("beforeend",'<style>.placeholder-add{justify-content:center;text-align:center}</style>'+ROW_ACTION_MENU_STYLES+this.themeStyles()+this.buttonThemeStyles()+this.iconHoverStyles()+this.groupListStyles());
    ...
    app.querySelector(".sort").addEventListener("selected",e=>{const value=e.target.value;if(value&&value!==this.sort){this.sort=value;this.render();}});
  ...
  filePill(file){const name=String(file.filename||""),parts=this.fileNameParts(name),url=this.signedFiles.get(file.id),content=`<span style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(parts.base)}</span><span style="flex:none;white-space:nowrap">${esc(parts.extension)}</span>`,style="display:inline-flex;align-items:center;max-width:160px;min-width:0;color:var(--primary-color);text-decoration:none";return url?`<button type="button" class="pill file-pill file-open" data-file-open="${esc(file.id)}" title="${esc(name)}" style="${style};min-height:0;padding:3px 8px;border:0">${content}</button>`:`<span class="pill file-pill" title="${esc(name)}" style="${style}">${content}</span>`;}
