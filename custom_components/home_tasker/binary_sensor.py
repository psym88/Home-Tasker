...
class TaskSensor(BinarySensorEntity):
    ...
    def extra_state_attributes(self):
        task = self._task
        return {"task_id": task["id"], **{k: task.get(k) for k in ("group_id", "assignee_user_id", "due_date", "start_date", "recurrence_mode", "frequency", "interval", "weekdays", "day_of_month", "month_of_year")}} if task else {}
