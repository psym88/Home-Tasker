...
- Add a Settings dialog above the task list with a collapsible Import / Export section.
- Export all groups, tasks, completion history, attachment metadata and attachment files as one ZIP archive.
- Import a current-format archive by clearing and replacing the complete existing data set after explicit confirmation and validation.
